...
public class BrowserTest extends SlimFixture {
    ...
    public boolean determineIfElementIsCoveredByChecking (String place, String criteria){
        /**
         * get element to check.
         * use the coordinates of said element to get another element.
         * check if element is the same as the original using criteria.
         */
        ...
        try {
            ...
            WebElement check = getElementFromPoint(element.getLocation());

            if (check != null) {
                switch (criteria) {
                    case "Text":
                        if (!check.getText().equals(element.getText())) {
                            result = true;
                        }
                        break;

                    /**
                     * Insert other possible criteria here
                     */
    ...
    private WebElement getElementFromPoint(Point point) {
        ...
        WebElement result = (WebElement) getSeleniumHelper().executeJavascript(
                "return document.elementFromPoint(arguments[0] - window.pageXOffset," +
